...
_BAND_WORDS = {"coarse": "gruesa", "medium": "media", "fine": "fina"}


def _band(en: str, es: str | None = None) -> dict[str, str]:
    """The expected band, in both languages.

    Every other user-facing string in an artifact is a bilingual record; this one was a bare English
    string and showed as English on the Spanish App route, under a Spanish label.

    Most bands are formulaic, a coarseness word and a measured range, so they are composed rather
    than restated. Anything that is not formulaic RAISES unless its Spanish is written out, which is
    what stops the next band from shipping in English by default.
    """
    if es is not None:
        return {"en": en, "es": es}
    match = re.fullmatch(r"(coarse|medium|fine), ([\d.]+) to ([\d.]+) m measured", en)
    if match is None:
...
def _real_cases() -> list[Case]:
    ...
    cases.append(
        Case(
            ...
            expected_band=_band(
                "the classical arm abstains on all six; the ratio arms answer",
                "el brazo clásico se abstiene en los seis; los brazos de razones responden",
            ),
            variants=_STANDARD_VARIANTS,
        )
    )
...
_SYNTHETIC: tuple[Case, ...] = (
    Case(
        ...
        expected_band=_band(
            "monotonic: coarser as the burden widens",
            "monótona: más gruesa a medida que el bordo se amplía",
        ),
        variants=_STANDARD_VARIANTS,
        dataset="synthetic",
    ...
    Case(
        ...
        expected_band=_band(
            "monotonic: finer as the powder factor rises",
            "monótona: más fina a medida que sube el factor de carga",
        ),
        variants=_STANDARD_VARIANTS,
        dataset="synthetic",
        licence="generated by this product; no measured data",
    ...
    Case(
        ...
        expected_band=_band(
            "coarse tail dominated by the block size, not by the powder factor",
            "cola gruesa dominada por el tamaño de bloque, no por el factor de carga",
        ),
        variants=_STANDARD_VARIANTS,
        dataset="synthetic",
        licence="generated by this product; no measured data",
    ...
    Case(
        ...
        expected_band=_band(
            ...
            "todo brazo se abstiene, con una razón",
        ),
    ...
    Case(
        ...
        expected_band=_band(
            "the generating arm recovers its own truth to within 1e-9",
            "el brazo generador recupera su propia verdad dentro de 1e-9",
        ),
        variants=_STANDARD_VARIANTS[:6],
        dataset="synthetic",
        licence="generated by this product; no measured data",
