...
pytest                                 # the tests run against the COMMITTED artifacts`}</code>
      ...
      <p>
        {es
          ? 'El horneado es una funcion pura del registro de casos, la version fijada del motor y la semilla, asi que repetirlo sobre un arbol sin cambios produce artefactos identicos byte a byte. Las pruebas corren contra los artefactos comprometidos y no contra un horneado en memoria, porque eso es lo que la web lee.'
          : 'The bake is a pure function of the case registry, the pinned engine version and the seed, so re-running it on an unchanged tree produces byte-identical artifacts. The tests run against the committed artifacts rather than a fresh in-memory bake, because that is what the web reads.'}
      </p>
