...
export default function Experiments() {
  ...
  return (
    <article className="fr-prose fr-prose-wide">
      ...
      <p className="fr-lede">
        {es
          ? 'Que se probo, por que cada caso esta en la matriz, y como responde el diseno cuando se mueve una sola palanca. Un caso sin razon cientifica es relleno.'
      ...
      <Callout variant="note" title={es ? 'Los cuatro pasan, y cada uno dispara solo donde puede' : 'All four pass, and each fires only where it can'}>
        {es
          ? 'Un bloque de control en cada caso pareceria minucioso y no significaria nada. Estos van adjuntos a los casos que pueden dispararlos.'
      ...
      <ul className="fr-list">
        <li>
          <b>{es ? 'Control negativo de geometria' : 'Geometry negative control'}</b>{' '}
          {es
            ? 'seis tiros cuya escala absoluta ninguna fuente publica. Todo modelo que necesite un volumen de roca se abstiene ahi con una razon, y una prueba verifica que ninguno respondio.'
        ...
        <li>
          ...
          {es
            ? 'seis disenos donde el taco excede el banco, asi que no hay columna de carga. TODOS los modelos se niegan, incluidos los que solo ven razones, porque la guarda esta al nivel del diseno y no del numero.'
        ...
        <li>
          <b>{es ? 'Control de extrapolacion' : 'Extrapolation control'}</b>{' '}
          {es
            ? 'cinco tiros de terreno bajo el minimo del corpus en su variable mas importante. Las 35 predicciones llevan el sello, comprobado y no supuesto.'
        ...
        <li>
          ...
          {es
            ? 'verdad generada por un modelo conocido, que la recupera con error cero. Prueba la instalacion, no la ciencia: si falla, ningun otro resultado de este sitio es confiable.'
      ...
      <h2>{es ? 'Respuesta del diseno' : 'Design response'}</h2>
      ...
      {benchmark ? (
        <>
          ...
          <p>
            {es
              ? 'Las filas de una misma campana no son muestras independientes: comparten macizo rocoso, equipo y operador de medicion. Por eso el benchmark excluye sitios completos, y por eso los intervalos remuestrean el sitio y no la fila.'
          ...
          <table className="fr-table">
            <thead>
              <tr>
                ...
                <th>{es ? 'proporcion del corpus' : 'share of the corpus'}</th>
              </tr>
            </thead>
            <tbody>
      ...
      <Refs ids={SECTION_REFS.experiments} label={es ? "Fuentes de esta pagina" : "Sources for this page"} />
    </article>
  );
}
...
function ResponseSurface({ es }: { es: boolean }) {
  ...
  return (
    <>
      <p>
        {es
          ? 'La superficie de respuesta del modelo clasico, recalculada en su navegador. Es la vista de compromiso que un ingeniero de diseno realmente quiere: mas explosivo compra roca mas fina, y un bordo mas amplio la pierde.'
      ...
      <LineChart
        x={burdens}
        series={series}
        xLabel={es ? 'bordo / diametro' : 'burden / hole diameter'}
      ...
      <div className="fr-inline-controls">
        ...
        <label className="fr-control">
          {es ? 'Diametro, mm' : 'Hole diameter, mm'}
          <input
            type="range"
            min={76}
            max={250}
      ...
      <p className="fr-fine">
        {es
          ? 'Todo lo demas se mantiene en el centro del corpus. Es un barrido, no un diseno: las campanas reales mueven varias cosas a la vez.'
...
function PowderSweep({
  ...
  return (
    <>
      <h3>{es ? 'El eje economico' : 'The economics axis'}</h3>
      <p>
        {es
          ? 'El explosivo es el lugar mas barato para fragmentar roca, y esta es la curva que dice cuanto mas fino compra cada kilogramo por metro cubico adicional. Todos los modelos deben bajar; si alguno sube, hay un signo equivocado.'
      ...
      <p className="fr-fine">
        {es
          ? 'Estos tiros son sinteticos y no tienen medicion, asi que el caso muestra una respuesta y no un puntaje. Decirlo asi evita que un bloque de metricas con ceros se lea como un fracaso del modelo.'
...
function ProtocolSlope({
  ...
  return (
    <>
      <p>
        {es
          ? 'Cada linea es un modelo, cruzando los tres protocolos. La pendiente es el hallazgo; una tabla obligaria al lector a calcularla.'
      ...
      <p className="fr-fine">
        ...
        {es ? 'es el mejor aprendido bajo el protocolo honesto; el nulo esta en' : 'is the best learned model under the honest protocol; the null sits at'}{' '}
