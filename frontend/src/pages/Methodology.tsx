...
export default function Methodology() {
  ...
  const tabs: SubTabDef[] = [
    { id: 'classical', label: es ? 'Clasicos' : 'Classical', content: <Classical es={es} /> },
    ...
    {
      id: 'statistical',
      label: es ? 'Estadisticos' : 'Statistical',
  ...
  return (
    <article className="fr-prose">
      ...
      <p className="fr-lede">
        {es
          ? 'Doce modelos en cuatro niveles, cada uno transcrito de una fuente primaria con cada simbolo definido, y cada uno con una nota de donde falla. Un nombre en un selector no es un metodo.'
      ...
      <Refs ids={SECTION_REFS.methodology} label={es ? "Fuentes de esta pagina" : "Sources for this page"} />
    </article>
  );
}
...
function Classical({ es }: { es: boolean }) {
  return (
    <section>
      <h2>{es ? 'El tamano medio clasico' : 'The classical mean size'}</h2>
      <p>
        {es
          ? 'La ecuacion de 1973 con la correccion de Cunningham por potencia del explosivo. Es cerrada, se evalua en microsegundos, y sigue siendo el estandar de la industria. Esa combinacion es exactamente la razon para probarla duro: un modelo asi de barato tiende a creerse.'
      ...
      <Equation
        ...
        caption={
          es
            ? 'Tamano medio de fragmento, en centimetros en la ecuacion original.'
      ...
      <p>
        {es
          ? 'Notese que V/Q es el reciproco del factor de carga, y por eso las dos formas publicadas de esta ecuacion, una en V/Q y otra en el factor de carga elevado a -0.8, coinciden en ese termino.'
      ...
      <h3>{es ? 'El indice de uniformidad' : 'The uniformity index'}</h3>
      <Equation
      ...
      <Callout variant="strong" title={es ? 'La trampa de unidades' : 'The unit trap'}>
        {es
          ? 'La fuente da el bordo en METROS y el diametro en MILIMETROS, asi que el B/d publicado vale cerca de 0.027 para un bordo de 4.5 m en un barreno de 165 mm. NO es la razon adimensional bordo sobre diametro que tabula el corpus, que es mil veces mayor. Leido asi, el termino principal se vuelve 2.2 menos 382 y el indice cae a unos -380. Una prueba del producto verifica que el indice cae en su banda publicada sobre los 91 tiros reconstruibles, y eso es lo que atrapa este error.'
      ...
      <ul className="fr-list">
        <li>{es ? 'Detonacion ideal. La detonica no ideal queda fuera de alcance.' : 'Ideal detonation. Non-ideal detonics is out of scope.'}</li>
        <li>
          {es
            ? 'Una sola columna de carga continua, que es lo que el ANFO es en estas mallas. El termino de distribucion de carga se reduce entonces a 0.1 elevado a 0.1, en vez de inventarse un reparto entre carga de fondo y de columna que el corpus no publica.'
        ...
        <li>
          {es
            ? 'El factor de tiempo y las correcciones de 2005 no estan tabuladas en ninguna fuente consultada. Valen 1 por defecto y las fija el usuario.'
...
function RockFactor({ es }: { es: boolean }) {
  return (
    <section>
      ...
      <p>
        {es
          ? 'Un solo numero adimensional carga con todo el "que clase de roca es esta". Su forma original era una busqueda de tres valores, que la propia literatura llama demasiado gruesa. La salida es un esquema de calificacion, y ahi es donde una coleccion de formulas elegiria una en silencio.'
      ...
      <Equation
        ...
        caption={es ? 'El indice de tronabilidad de Lilly.' : 'Lilly’s blastability index.'}
      />

      <Callout variant="honest" title={es ? 'Dos tablas bajo la misma atribucion' : 'Two tables under the same attribution'}>
        {es
          ? 'Dos fuentes primarias publican tablas de calificacion atribuidas a Lilly, y no son la misma tabla. El termino de resistencia es 0.05 por la resistencia a compresion simple en una, y esa resistencia dividida por 3 o por 5 segun el modulo en la otra. Con 100 MPa eso da 5 frente a 33.3 o 20, lo que mueve el indice hasta 14 puntos y el factor de roca hasta 0.85. El tamano predicho es lineal en ese factor.'
      ...
      <p>
        {es
          ? 'Los tres esquemas se muestran lado a lado, con su fuente en cada uno. Presentar uno como EL factor de roca ocultaria la subjetividad que es el contenido honesto de este parametro.'
      ...
      <p>
        {es
          ? 'Ambas fuentes dicen que el factor de roca "se estimo para cada tiro" y ninguna publica un valor. Invirtiendo la ecuacion de tamano medio sobre una prediccion publicada se recupera:'
      ...
      <p>
        {es
          ? 'Los valores recuperados apenas se mueven dentro de cada sitio: la mayor dispersion es 3.7 por ciento, que es lo que produce redondear las predicciones publicadas a dos decimales. Un error en la reconstruccion geometrica los dispersaria. Ese unico resultado valida la geometria, resuelve el desacuerdo de exponentes y recupera una constante que la literatura omitio.'
      ...
      <Callout variant="note" title={es ? 'Una inversion, nombrada en vez de suavizada' : 'One inversion, named rather than smoothed'}>
        {es
          ? 'Los factores recuperados correlacionan con el modulo de Young en 0.87, no cerca de uno, y hay una inversion que vale la pena nombrar: la roca mas rigida del corpus, un esquisto plegado de 60 GPa, queda por debajo de los carbonatos de 45 GPa. La rigidez no es lo unico que carga el factor de roca.'
...
function Distributions({ es }: { es: boolean }) {
  return (
    <section>
      <h2>{es ? 'De un tamano medio a una curva' : 'From a mean size to a curve'}</h2>

      <h3>{es ? 'Dos parametros' : 'Two parameters'}</h3>
      <Equation
        tex={String.raw`R(x) = \exp\!\left[-0.693\left(\frac{x}{x_{50}}\right)^{n}\right]`}
        caption={es ? 'Fraccion retenida sobre una malla x; lo que pasa es uno menos eso.' : 'Fraction retained above a mesh x; passing is one minus that.'}
      />
      <p>
        {es
          ? 'El 0.693 es el logaritmo natural de dos, y es lo que hace que x50 sea el tamano al 50 por ciento pasante en vez del tamano caracteristico. El caracteristico, por el que pasa el 63.2 por ciento, vale x50 dividido por 0.693 elevado a 1/n.'
      ...
      <h3>{es ? 'Tres parametros' : 'Three parameters'}</h3>
      <Equation
      ...
      <p>
        {es
          ? 'El limite superior explicito corrige la cola gruesa que la forma de dos parametros equivoca, y la curvatura adicional corrige la rama de finos.'
      ...
      <Callout variant="honest" title={es ? 'Un contraejemplo que viaja con el modelo' : 'A counter-example that travels with the model'}>
        {es
          ? 'La misma fuente que recomienda la forma de tres parametros reporta un sitio donde la mas simple ajusto mejor: sobre 24 tiros en una mina de bauxita medidos por analisis de imagen, el rango de la forma de dos parametros quedo mas cerca de la medicion. Es mas adaptable en general y no mejor en todas partes.'
      ...
      <h3>{es ? 'La composicion de dos ramas' : 'The two-branch composition'}</h3>
      <p>
        {es
          ? 'Dos mecanismos actuan a la vez: la fractura por traccion produce los fragmentos gruesos y la fractura por corte en la zona triturada alrededor del barreno produce los finos. La rama gruesa es el modelo clasico; la fina modifica los parametros de la distribucion.'
      ...
      <Callout variant="strong" title={es ? 'La estructura tiene fuente, las constantes no' : 'The structure is sourced, the constants are not'}>
        {es
          ? 'Los articulos que introdujeron estos modelos son actas de congreso que no se poseen para este trabajo; lo que si se tiene es una descripcion del mecanismo. Asi que el tamano de cruce, la uniformidad de la rama fina y la fraccion de finos los fija el usuario, con valores por defecto declarados como puntos de partida plausibles y no como valores publicados. Cada curva que este modelo devuelve lleva esa distincion.'
...
function Statistical({ es }: { es: boolean }) {
  return (
    <section>
      ...
      <p>
        {es
          ? 'Los 97 tiros se separan por analisis de conglomerados en dos grupos: 35 de modulo alto, con media 51.14 GPa, y 62 de modulo bajo, con media 17.22 GPa. El analisis de la propia fuente muestra por que funciona: el modulo domina la separacion, y la razon espaciamiento sobre bordo no influye en absoluto en la pertenencia al grupo.'
      ...
      <Equation
        ...
        caption={es ? 'La funcion discriminante. Por encima de 11.821 es el grupo de modulo alto.' : 'The discriminant function. Above 11.821 is the high-modulus group.'}
      />

      <Callout variant="note" title={es ? 'Se reproduce exactamente' : 'It reproduces exactly'}>
        {es
          ? 'Cero errores de asignacion en los 97 tiros de entrenamiento y en los 12 de validacion. Los dos grupos quedan perfectamente separados, con maximo 10.318 en el grupo bajo y minimo 13.067 en el alto. Eso importa mas que la contabilidad: significa que el enrutador es una compuerta real y no una consulta de etiqueta, asi que las ecuaciones por grupo funcionan sobre disenos nuevos.'
      ...
      <Equation
        ...
        caption={es ? 'Grupo 1, modulo alto. R2 0.708 sobre 35 tiros.' : 'Group 1, high modulus. R2 0.708 on 35 blasts.'}
      />
      <Equation
        ...
        caption={es ? 'Grupo 2, modulo bajo. R2 0.739 sobre 62 tiros.' : 'Group 2, low modulus. R2 0.739 on 62 blasts.'}
      />

      <Callout variant="note" title={es ? 'Un aparente error de unidades que no lo es' : 'An apparent unit error that is not one'}>
        {es
          ? 'Los dos coeficientes principales difieren por un factor de 347 y ambas ecuaciones devuelven metros. Parece un error y no lo es: los exponentes del modulo difieren en 0.6 sobre un rango de 9.57 a 60 GPa, y el termino del modulo absorbe la diferencia. Se comprobo numericamente antes de aceptar los coeficientes.'
      ...
      <p>
        {es
          ? 'Ambos articulos imprimen estas ecuaciones Y una tabla de predicciones hechas con ellas. Recalcular las ecuaciones y puntuar sobre las mismas filas da un resultado materialmente mejor que cualquiera de las dos tablas: 0.854 contra el 0.747 propio del articulo de 2010, y 0.827 contra el 0.708 del de 2012. En las cuatro filas donde los dos articulos se contradicen, el recalculo cae sobre la cifra de 2010 las cuatro veces.'
      ...
      <p>
        {es
          ? 'Una fila no coincide con ninguno de los dos por un factor de dos, y dos modelos independientes se apartan de la fuente en la misma direccion sobre esa fila. Se probo la hipotesis de que una celda de entrada estuviera mal: ningun valor unico reconcilia ambos modelos. La fila viaja con sus entradas publicadas y con su inconsistencia registrada.'
...
function Learned({ es }: { es: boolean }) {
  return (
    <section>
      ...
      <p>
        {es
          ? 'Tres de los cinco estan completamente especificados en sus fuentes, hasta el ancho de la capa oculta, el algoritmo de entrenamiento, la normalizacion y los hiperparametros. Eso los hace reproducciones y no reimplementaciones, y es lo que permite que una reproduccion pueda discrepar del articulo de forma significativa.'
      ...
      <ul className="fr-list">
        ...
        <li>{es ? 'entradas y objetivo normalizados por minimo y maximo' : 'inputs and target normalised by minimum and maximum'}</li>
      ...
      <Callout variant="strong" title={es ? 'La reproduccion no alcanza el puntaje publicado' : 'The reproduction does not reach the published score'}>
        {es
          ? 'A lo largo de treinta semillas, la varianza explicada de la reproduccion sobre el conjunto de validacion publicado va de 0.167 a 0.636, con mediana cerca de 0.34. El 0.910 publicado queda por encima de todas ellas y ninguna semilla llega siquiera a 0.7. El deficit se concentra en las dos filas que la propia fuente reporta como sus mas inestables, con coeficientes de variacion de 0.56 y 0.76 en sus propias tablas. La afirmacion es estrecha y verificable: el puntaje publicado no es robusto a la semilla.'
      ...
      <p>
        {es
          ? 'La arquitectura es generosa respecto de los datos: siete unidades ocultas sobre 62 tiros son 64 parametros libres, y nueve sobre 35 son 82. Ambos recuentos viajan en cada prediccion.'
      ...
      <p>
        {es
          ? 'Dos fuentes ajustan la regresion por vectores de soporte sobre este mismo corpus y llegan a conclusiones opuestas sobre el nucleo. Ambas se incluyen; promediarlas borraria el desacuerdo, que es la parte interesante. El bosque aleatorio, la potenciacion por gradiente y el ensamble apilado se reproducen con los hiperparametros finales publicados, incluido el sobreajuste que la propia fuente reporta, que se deja visible en vez de corregirse.'
      ...
      <Callout variant="note" title={es ? 'Lo que no se reprodujo, y por que' : 'What was not reproduced, and why'}>
        {es
          ? 'Un hibrido de 2025 que combina una red convolucional, una maquina de vectores de soporte por minimos cuadrados y un optimizador tipo Newton-Raphson reporta cifras fuertes sobre un superconjunto de este corpus. No se reproduce: la regla de actualizacion del optimizador no es transcribible con confianza desde la copia disponible, y una aproximacion casera bajo ese nombre seria un metodo fabricado. Aparece citado con sus cifras publicadas, no como un modelo de este producto.'
...
function Protocol({ es }: { es: boolean }) {
  return (
    <section>
      ...
      <p>
        {es
          ? 'El estado del arte de 2025 sobre este corpus reporta una varianza explicada de 0.943 desde una particion aleatoria 80/20 de 97 filas. Diecisiete de esas filas duplican el vector de caracteristicas de otra fila, asi que una particion aleatoria coloca duplicados a ambos lados por construccion. El mismo articulo registra que probo validacion cruzada y la quito porque "el modelo con validacion cruzada tuvo un efecto de prediccion pobre sobre el conjunto de prueba", que es exactamente el sintoma que esto predice.'
      ...
      <table className="fr-table">
        ...
        <tbody>
          <tr>
            ...
            <td>{es ? 'particion aleatoria con semilla' : 'seeded random partition'}</td>
          ...
          <tr>
            ...
            <td>{es ? 'aisla el efecto de los duplicados' : 'isolates the duplicate effect alone'}</td>
          </tr>
          <tr>
            <td>{es ? 'dejar un sitio fuera' : 'leave one site out'}</td>
            <td>{es ? 'una campana completa fuera, diez pliegues' : 'a whole campaign held out, ten folds'}</td>
      ...
      <p>
        {es
          ? 'El tercero es la pregunta del profesional. Las filas de una misma campana comparten macizo rocoso, equipo de perforacion, suministro de explosivo y operador de medicion, y una sola cantera aporta 22 de las 97 filas.'
      ...
      <Callout variant="honest" title={es ? 'El criterio de descarte, declarado antes de correr' : 'The kill criterion, declared before the run'}>
        {es
          ? 'El nivel aprendido cuenta como generalizable entre sitios solo si la varianza explicada del mejor modelo aprendido bajo dejar-un-sitio-fuera es a la vez positiva y al menos 0.10 sobre la del modelo nulo. Ambas mitades son necesarias, y la segunda se agrego DESPUES de la primera corrida, porque esa corrida atrapo al criterio fallando en medir su propio objeto: entrego un mejor modelo aprendido en -0.034 contra un nulo en -0.216, y la regla declaro exito. Una compuerta que puede aprobar con dos fracasos no es una compuerta.'
